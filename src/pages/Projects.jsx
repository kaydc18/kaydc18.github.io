...
const renderProjects = (projects, expandedProject, toggleProject) => {
  return projects.map((project, index) => (
    <li key={index} className="mb-10 ms-4 flex flex-col gap-1 justify-start">
      ...
      <h2 className="text-3xl font-family-serif text-fuchsia-900">
        {project.title}
      </h2>
      ...
      <div>
        <Button
          ...
          buttonIcon={expandedProject === index ? faChevronUp : faChevronDown}
            ...
            project.details.images.map((image, imageIndex) => (
              <div key={imageIndex}>
                <figure className="group cursor-pointer">
                  <a
                    href={`#lightbox-${index}-${imageIndex}`}
                    className="block relative overflow-hidden rounded-lg border-2 border-transparent hover:border-fuchsia-500 transition-all duration-300"
                  >
                ...
                <div
                  id={`lightbox-${index}-${imageIndex}`}
                  className="hidden target:block fixed inset-0 bg-black/75 z-50 p-8"
                >
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="relative max-w-full max-h-full">
                      <a
                        href="#"
                        className="bg-white hover:bg-gray-100 px-3 py-1 text-black absolute -right-10 -top-5 rounded z-10 transition-colors duration-200"
                      >
                        ✕
                      </a>
                      <img
      ...
      <div className="mt-5 flex flex-wrap justify-start gap-2">
        {project.tags.map((tag, tagIndex) => (
          <span
            key={tagIndex}
            className="bg-salmon-200 text-salmon-900 px-2 py-1 rounded-full text-sm"
          >
            {tag}
          </span>
        ))}
...
export default function Projects() {
  ...
  return (
    <section className="mx-auto my-auto max-w-5xl flex flex-col items-start justify-center gap-14 md:p-4 p-3">
      <h1 className="text-5xl font-family-serif text-gray-900">
        Projects Page
      </h1>
