...
const VARIANT_STYLES = {
  primary:
    "bg-fuchsia-900 text-white border-fuchsia-600 drop-shadow-primary hover:bg-fuchsia-300 hover:text-gray-900",
  secondary:
    "bg-white text-fuchsia-900 border-fuchsia-900 drop-shadow-secondary hover:bg-gray-100 ",
...
function Button({
  ...
  const computedAriaLabel = disabled
    ? `${buttonText} button disabled`
    : !buttonIcon
      ? `click for ${buttonText}`
      : ariaLabel;
  ...
  return (
    <button
      onClick={handleClick}
      className={`rounded-md border-2 text-base font-sans ${getPadding()} ${VARIANT_STYLES[effectiveVariant]}`
        .trim()
        .replace(/\s+/g, " ")}
      disabled={disabled}
